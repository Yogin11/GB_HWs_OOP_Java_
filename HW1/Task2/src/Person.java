public class Person {
    ...
    public void checkWardContents(Wardrobe obj){
        if (!obj.getStatus()) {
        System.out.println( "Всего вещей в шкафу - " +obj.amount + " шт. : " + obj.getWardContents());
        }
        else {
    ...
    public void takeClothOut(String somecloth, Wardrobe obj){
        if (!obj.getStatus()) {
            if (obj.getWardContents().contains(somecloth)) {
                System.out.println(this.name +  " берет из шкафа " + somecloth);
                obj.leaveContents(somecloth);
            }
